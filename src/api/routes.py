...
def addCarrito():
    ...
    request_body_cart = request.get_json()
    OrderDetailID = request_body_cart["OrderDetailID"]
    OrderID = request_body_cart["OrderID"]
    ProductID = request_body_cart["ProductID"]
    Quantity = request_body_cart["Quantity"]
    SubTotal = request_body_cart["SubTotal"]
    Discount = request_body_cart["Discount"]
    Tax = request_body_cart["Tax"]
    Total = request_body_cart["Total"]

    record = OrderDetail.query.filter_by(OrderDetailID=OrderDetailID, OrderID=OrderID, ProductID=ProductID, Quantity=Quantity, SubTotal=SubTotal, Discount=Discount, Tax=Tax, Total=Total)
    if record is None:
        record = OrderDetail(OrderDetailID=OrderDetailID, OrderID=OrderID, ProductID=ProductID, SubTotal=SubTotal, Discount=Discount, Tax=Tax, Total=Total, Quantity=request_body_cart["Quantity"])
        db.session.add(record)
    else:
        record.Quantity = request_body_cart["Quantity"]
